 ...
 *
 * <ol>
 *   <li>Instantiate the calibration with a default value, and reference to the wrangler
 *   <li>At runtime, use the get() method to read the calibrated value. The returned value may
 *       change depending on what the wrangler has overwritten.
 * </ol>
 */
public class Calibration {
   ...
   *
   * @param name_in String for the name of the calibration. Best to make it the same of the variable
   *     name.
   ...
   *     outside this range, a WARNING: will be thrown and the calibrated value will be capped at
   *     the minimum.
   ...
   */
  public Calibration(
      String name_in, String units, double default_val_in, double min_in, double max_in) {
  ...
  private void commonConstructor() {
    ...
    calCurValueTopic.setProperties(
        "{ \"default_val\" : " + Double.toString(this.default_val) + "}");
  ...
  private double limitRange(double in) {
    ...
    if (in < min_cal) {
      DriverStation.reportWarning(
          "WARNING: Calibration: Requested value for "
              + name
              + " is too small. Setting value to minimum value of "
              + Double.toString(min_cal),
          false);
      temp = min_cal;
    } else if (in > max_cal) {
      DriverStation.reportWarning(
          "WARNING: Calibration: Requested value for "
              + name
              + " is too large. Setting value to maximum value of "
              + Double.toString(max_cal),
          false);
      temp = max_cal;
    } else {
      temp = in;
   ...
   * method. <br>
   * <br>
   * The intent of this functionality is to allow software the ability to run special procedures if
   * the value changes (ex: update and reset PID tune values). The acknowledgement makes sure any
   * code that cares about cal value changes can always see when a new one occurs. <br>
   * <br>
   * Note you never technically have to worry about this, for most cases. If you're reading the cal
   * value every loop, you'll probably be ok. The bigger thing is when you need to not only read the
   * cal value, but also start over some other piece of code or something like that. <br>
   * <br>
   ...
   * "always seeing changes in cal values". There's lots of ways to break it, especially if the cal
   * updater and the code that cares are in different theads. Ah well. The redeeming characteristic
   * of this case is the cal values <i>hopefully</i> won't change too fast, because they're human
   * entered. This <i>should</i> mean that we always see the cycle of change-process-acknowledge
   * cleanly, but if we don't, well.... hmmm... let's just cross our fingers for now. <br>
   * <br>
   * Isn't software fun?
   *
  ...
  public void acknowledgeValUpdate() {
    if (is_updated) System.out.println("Info: Calibration " + this.name + " acknowledged update ");

    is_updated = false;
  }

  /**
   * Get the default value of the calibration. Will not be overridden by wrangler. Do not use unless
   * you're certain you don't wan't any updates the wrangler may apply to this calibration.
   *
  ...
  /**
   * Set a new value to override the present calibration. Value will be limited to allowable min/max
   * range for this calibration.
   *
  ...
  String getCurValueTopic() {
    return "/Calibrations/" + this.name + "/curValue";
  }

  String getDesValueTopic() {
    return "/Calibrations/" + this.name + "/desValue";
  }
}
