 ...
 * itself as "done". <br>
 * <br>
 * All events on the timeline must implement the functions defined here. Constructor is for
 * performing all user-specific actions during sequencer setup. User update is the periodic update
 * function which the sequencer will call as long as the event is active. isDone() signals the
 * sequencer this event no longer needs to run, while isTriggered() is used for child events to
 * signal they need to be run. Calculation of triggered/done conditions shoudl be performed within
 * those methods. Do not rely upon update() to be called to evaluate those conditions. Finally, the
 * forceStop method is required to be implemented - ordinarily, the event should return the robot to
 * a safe state before declaring itself done. However, if the autonomous sequence is interrupted
 * before the event has a chance to finish naturally, this function will be called to shut down the
 * event properly. This means doing things like stopping motors or resetting counters, whatever
 * floats your boat (safely).
 *
 * @author Chris Gerth
 */
