...
class AutoDiscoveredSignal {
   ...
   *
   * @param sourceField_in Java Reflection Field object found during recursive searching for @Signal
   *     annotated objects
   * @param sourceFieldParentObj_in Object that should get its value read at runtime to populate the
   *     signal's value
   * @param name Signal name
   * @param units Signal units string
   */
  AutoDiscoveredSignal(
      Field sourceField_in, Object sourceFieldParentObj_in, String name, String units) {
  ...
  /**
   * Sample the value out of the referenced object for the ADS, and add it as a new sample into the
   * signal.
   *
  ...
  void addSample(double time) {
    ...
    try {
      if (type == int.class) {
        ...
        try {
          Field valField =
              enumObj.getClass().getDeclaredField("value"); // most casserole enums define a value
        ...
        System.out.println(
            "WARNING: Signal "
                + sig.getName()
                + " cannot be populated from underlying type "
                + type.getName()
                + ". It is not yet supported, but you could add support!");
      }
    } catch (IllegalAccessException e) {
      System.out.println(
          "WARNING: Signal "
              + sig.getName()
              + " threw Illegal Access while we were attempting to read it.");
      System.out.println(e);
    } catch (NullPointerException e) {
      System.out.println(
          "WARNING: Signal "
              + sig.getName()
              + " threw Null Pointer while we were attempting to read it.");
      System.out.println(e);
    }
  }
}
