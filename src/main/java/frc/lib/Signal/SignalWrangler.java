...
public class SignalWrangler {
  ...
  public int register(Signal sig_in) {
    ...
    if (registeredSignals.contains(sig_in)) {
      DriverStation.reportWarning(
          "[SignalWrangler] WARNING: "
              + sig_in.name
              + " has already been added to the signal wrangler. Nothing done.",
          false);
      ret_val = -1;
    } else {
  ...
  /**
   * Set of all java Reflection objects that we've analyzed for whether they have @Signal annotation
   * or not In the event that two objects reference each other, this should help break an
   * infinite-recursion case, since each unique object only needs to be checked once.
   */
  Set<Object> checkedObjects;

  /**
   * Recursively-called signals-finding function. Starting at some object `root` it traverses each
   * field (ie, variable or object or whatever) declared withing the class of that object, finds the
   * objects of each of those fields, and does none of two things: 1) Check if it's annotated to be
   * a signal 2) Otherwise, call the function recursively on the new "child" object. Recursion stops
   * if we hit #1, or if the object's class is not "frc.robot" - this should help keep search time
   * reasonable, since this happens at runtime at robot init. Note this should NOT be called at
   * periodic runtime.... because a) it's not tested that way and b) recursion.
   *
   * @param root
   * @param prefix
   */
  void findAllAnnotatedSignals(Object root, String prefix) {
    ...
    if (rootPkg.toString().contains("frc.sim")
        || rootPkg.toString().contains("frc.robot")
        || rootPkg.toString().contains("frc.wrappers")) {
      ...
      for (Field field : rootClass.getDeclaredFields()) {
        ...
        if (field.isAnnotationPresent(frc.lib.Signal.Annotations.Signal.class)) {
          ...
          frc.lib.Signal.Annotations.Signal ann =
              field.getAnnotation(frc.lib.Signal.Annotations.Signal.class);
  ...
  /**
   * Special thanks to oblarg and his oblog for help on impelmenting this. Entrypoint to set up
   * required variable sets, and traverse rootContainer and its children to find
   * all @Signal-annottated fields, and add a new signal for each of them. Call this in robotInit(),
   * and after all classes which contain @Signals have been instantiated.
   *
   * @param rootContainer Object to start the traversal on. Usually just "this" for when called in
   *     Robot.java.
   */
  public void registerSignals(Object rootContainer) {
    ...
    System.out.println(
        "[Data Server]: Registered "
            + Integer.toString(autoSig.size())
            + " signals from annotations");
  ...
  /**
   * Periodic call function to sample a single value from all annotation-created Signals Should be
   * called at the end of each periodic function.
   */
