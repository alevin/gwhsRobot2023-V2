...
public class Signal {
  ...
  public Signal(String name_in, String units_in) {
    ...
    // The goal of a signal is to record the value of a variable every loop, for debugging down to
    // loop-to-loop changes
    // Therefor we do want to send all vlaues over the network, and we do want to keep any
    // duplicates.
    nt4ValPublisher =
        nt4ValTopic.publish(PubSubOption.sendAll(true), PubSubOption.keepDuplicates(true));
  ...
  /**
   * Adds a new sample to the signal queue. It is intended that the controls code would call this
   * once per loop to add a new datapoint to the real-time graph.
   *
   * <p>The boolean version converts true to 1.0 and false to 0.0.
   *
   * @param time_in
   * @param value_in
   */
  ...
  public String getNT4ValueTopicName() {
    return SignalUtils.nameToNT4ValueTopic(this.name);
  }
}
