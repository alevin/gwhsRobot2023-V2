...
public class RealTalonFX extends AbstractSimmableMotorController {
    ...
    while (!success) {
      ...
      var err2 =
          _talon.configSelectedFeedbackSensor(
              TalonFXFeedbackDevice.IntegratedSensor, 0, TIMEOUT_MS);
      ...
      var err10 =
          _talon.configStatorCurrentLimit(
              new StatorCurrentLimitConfiguration(true, 40, 40, 0), TIMEOUT_MS);

      // Reduce CAN bus rates on things we don't quite carea bout
      var err11 =
          _talon.setStatusFramePeriod(
              StatusFrameEnhanced.Status_1_General, 21, TIMEOUT_MS); // Applied motor output, faults
      var err12 =
          _talon.setStatusFramePeriod(
              StatusFrameEnhanced.Status_2_Feedback0, 61, TIMEOUT_MS); // Position/Velocity
      var err13 =
          _talon.setStatusFramePeriod(
              StatusFrameEnhanced.Status_3_Quadrature, 9999, TIMEOUT_MS); // Quadrature - unused
      var err14 =
          _talon.setStatusFramePeriod(
              StatusFrameEnhanced.Status_4_AinTempVbat,
              200,
              TIMEOUT_MS); // Includes input supply voltage, which we might care about
      var err15 =
          _talon.setStatusFramePeriod(
              StatusFrameEnhanced.Status_8_PulseWidth, 9999, TIMEOUT_MS); // No idea, not used
      var err16 =
          _talon.setStatusFramePeriod(
              StatusFrameEnhanced.Status_10_MotionMagic,
              9999,
              TIMEOUT_MS); // Not using motion magic
      var err17 =
          _talon.setStatusFramePeriod(
              StatusFrameEnhanced.Status_12_Feedback1,
              9999,
              TIMEOUT_MS); // No external feedback sensors connected
      var err18 =
          _talon.setStatusFramePeriod(
              StatusFrameEnhanced.Status_14_Turn_PIDF1,
              9999,
              TIMEOUT_MS); // No no auxilary PID used
  ...
  public void setClosedLoopGains(double p, double i, double d) {
    ...
    i =
        (CMD_PER_V)
            * RevtoCTRENativeUnits(
                Units.radiansToRotations(i)); // CTRE needs this in * 1000 ms (or * 1 sec)
    d =
        (CMD_PER_V)
            * RevtoCTRENativeUnits(
                Units.radiansToRotations(d)); // CTRE needs this in / 1000 ms (or / 1 sec)
  ...
  public void setClosedLoopCmd(double velocityCmd_radpersec, double arbFF_V) {
    var arbFF_demand =
        arbFF_V / 12.0; // CTRE wants this in the range [-1, 1].  TODO use bbattery voltage?
    var velCmdRPM = Units.radiansPerSecondToRotationsPerMinute(velocityCmd_radpersec);
    _talon.set(
        TalonFXControlMode.Velocity,
        RPMtoCTRENativeUnits(velCmdRPM),
  ...
  public void follow(Object leader) {
    if (leader.getClass() == RealTalonFX.class) {
      ...
      throw new IllegalArgumentException(
          leader.getClass().toString()
              + " cannot be followed by a "
              + RealTalonFX.class.toString());
    }
  }
